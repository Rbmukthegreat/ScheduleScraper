...
DISTANCE_BETWEEN_LINES = 72
DISTANCE_BETWEEN_DAYS = 344

def get_height(start, end, typ):
    height = int((int(DISTANCE_BETWEEN_LINES*(start - 8))+174+int(72*(end - 8)))/2) - 15
    if typ != "":
        height -= 10
    return height
...
def draw_class(classname, typ, times, img1, fnt, n):
    ...
    for i in range(len(times)):
        ...
        shape = ((150 + DISTANCE_BETWEEN_DAYS*i, int(DISTANCE_BETWEEN_LINES*(start - 8))+87), (150+344*(i+1), int(72*(end - 8)) + 87)) 
...
def draw_schedule(classes):
    ...
    for i in range(14):
        img1.text((30, DISTANCE_BETWEEN_LINES*i+75), time[i], font=fnt, fill=(0, 0, 0, 128))  
        img1.line(((100, DISTANCE_BETWEEN_LINES*i + 87), (1920, 72*i + 87)), fill="black", width=2)
    for i in range(5):
        img1.text((DISTANCE_BETWEEN_DAYS*i+300, 30), days[i], font=fnt, fill=(0, 0, 0, 128))
